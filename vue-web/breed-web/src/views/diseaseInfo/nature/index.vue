    ...
    <div class="table-container">
      <el-table ref="revenueTable"
        ...
        <el-table-column label="编号" width="100" align="center" >
          <template slot-scope="scope">{{ scope.row.nid }}</template>
        </el-table-column>
        <el-table-column label="病理性质" align="center">
...
<script>
import {fetchList, deleteNature} from '@/api/nature';
...
export default {
  ...
  methods: {
    ...
    /**跳转添加 **/
    handleAddBreed() {
      this.$router.push('/diseaseInfo/addNature');
    },
    /**跳转编辑 **/
    handleUpdate(index, row) {
      this.$router.push({path: '/diseaseInfo/updateNature', query: {id: row.nid}});
    },
    ...
    handleDelete(index, row) {
      this.$confirm('是否要删除该养殖记录?', '提示', {
        ...
        deleteNature(row.nid).then(response => {
          this.$message({
            type: 'success',
            message: '删除成功!'
          });
